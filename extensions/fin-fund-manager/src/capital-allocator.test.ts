...
import type { FundConfig, StrategyProfile } from "./types.js";
...
describe("CapitalAllocator", () => {
  ...
  it("allocates to a single L3 strategy", () => {
    ...
    expect(result[0]!.capitalUsd).toBeLessThanOrEqual(30000); // maxSingleStrategyPct
  });
  ...
  it("total allocation does not exceed maxTotalExposurePct", () => {
    ...
    expect(totalAllocated).toBeLessThanOrEqual(70000 + 1); // maxTotalExposurePct + float tolerance
  });
  ...
  it("applies correlation constraints", () => {
    ...
    // s1 and s2 are highly correlated
    ...
    // s1+s2 combined allocation should be smaller with correlation constraint
