...
describe("CorrelationMonitor", () => {
  ...
  it("computes pairwise correlations", () => {
    ...
    curves.set("s2", [0.01, 0.02, -0.01, 0.03, 0.01]); // identical
    curves.set("s3", [-0.01, -0.02, 0.01, -0.03, -0.01]); // inverse
    ...
    // High correlation: s1-s2 (positive) and s1-s3, s2-s3 (negative, abs ≥ 0.7)
