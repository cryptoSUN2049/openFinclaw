...
describe("Leaderboard", () => {
  ...
  it("ranks strategies by confidence-adjusted score", () => {
    ...
    // L1: 2.0 * 0.3 = 0.6
    // L2: 1.5 * 0.7 = 1.05
    expect(result[0]!.strategyId).toBe("L2"); // 1.05
    expect(result[1]!.strategyId).toBe("L3"); // 1.0
    expect(result[2]!.strategyId).toBe("L1"); // 0.6
  ...
  it("applies walk-forward verification bonus", () => {
    ...
    expect(verified.confidenceMultiplier).toBe(0.8); // 0.7 + 0.1 bonus
