...
describe("FundManager", () => {
  ...
  it("builds profiles from strategy records", () => {
    ...
    expect(profiles).toHaveLength(2); // KILLED filtered out
  ...
  it("allocates capital to eligible strategies", () => {
    ...
    expect(total).toBeLessThanOrEqual(70000 + 1); // maxTotalExposurePct
  ...
  it("generates leaderboard", () => {
    ...
    // L3 should rank higher due to confidence multiplier
    expect(lb.find((e) => e.strategyId === "s2")!.confidenceMultiplier).toBe(1.1); // 1.0 + 0.1 WF bonus
  });
  ...
  it("evaluates fund risk", () => {
    ...
    expect(risk.riskLevel).toBe("caution"); // 5% drawdown
  ...
  describe("rebalance (full cycle)", () => {
    it("runs end-to-end rebalance", () => {
      ...
      // Allocations — s1 (L2) and s2 (L3) should get capital
      expect(result.allocations.length).toBeGreaterThan(0);

      // Leaderboard — should exclude L0
      expect(result.leaderboard.length).toBe(2);

      // Risk
      expect(result.risk.riskLevel).toBeDefined();

      // Promotions — L0 should be eligible for L1
    ...
    it("incorporates correlation data in rebalance", () => {
      ...
      equityCurves.set("s2", [0.01, 0.02, -0.01, 0.03, -0.02, 0.01, 0.02]); // identical

      const result = fm.rebalance(records, undefined, equityCurves);

      // Both should get allocations but total should be constrained
