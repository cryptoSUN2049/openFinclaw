...
<p>
  ...
  <a href="#核心特性">特性</a> ·
  <a href="#finclaw-commons-开放金融智能共同体">FinClaw Commons</a> ·
  <a href="#快速开始">快速开始</a> ·
  <a href="#路线图">路线图</a> ·
  <a href="#参与贡献">贡献</a> ·
...
- **个人投资者** — 想要一个 7x24 在线的 AI 伙伴，记住你的投资组合、目标和风险偏好
- **加密交易者** — 需要跨多个交易所的全天候监控，配备安全护栏
- **个人理财爱好者** — 想要主动式预算管理、税务规划和退休模拟
- **量化开发者** — 想在开放生态中贡献和分享交易策略
- **理财顾问** — 想用 AI 分析和客户工具增强专业实践
- **开源信仰者** — 认为金融智能应该是公共资源，而非围墙花园
...
- **开放生态** — 不只是开源代码，更是一个开放的 **FinClaw Commons**，社区共享技能、策略、数据连接器等金融智能资源。
...
## FinClaw Commons (开放金融智能共同体)

**FinClaw Commons** 是我们的开放金融智能生态 —— 一个社区驱动的共同体，每个人都可以贡献、发现和分享金融构建模块。它远不只是一个技能商店。
...
│  │  金融技能 │  │  交易策略 │  │ 数据连接器│  │ Agent人格 │    │
│  │          │  │          │  │          │  │          │    │
│  │组合分析  │  │定投计划  │  │交易所API │  │价值投资者│    │
│  │税务规则  │  │网格策略  │  │链上数据  │  │日内交易员│    │
│  │预算管理  │  │套利模型  │  │新闻/NLP  │  │风控经理  │    │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘    │
│                                                              │
│  ┌──────────┐  ┌──────────┐  ┌────────────────────────┐    │
...
│  │ 工作空间  │  │  知识包   │  │      合规规则集        │    │
│  │          │  │          │  │                        │    │
│  │加密交易  │  │各国税法  │  │US-FATCA  EU-MiFID      │    │
│  │退休规划  │  │监管规则  │  │CN-CSRC   JP-FSA        │    │
│  │个人预算  │  │历史案例  │  │各国加密税务规则         │    │
│  └──────────┘  └──────────┘  └────────────────────────┘    │
└──────────────────────────────────────────────────────────────┘
```
...
| 维度                    | 你可以贡献什么                                | 格式                 | 举例                                                        |
| ----------------------- | --------------------------------------------- | -------------------- | ----------------------------------------------------------- |
| **技能 Skills**         | 金融分析方法、规划技术                        | `SKILL.md`           | 投资组合再平衡技能、税务亏损收割技能                        |
| **策略 Strategies**     | 可回测、可执行的交易策略                      | Strategy YAML + 逻辑 | BTC 周定投、均值回归网格、跨所套利                          |
| **连接器 Connectors**   | 市场/链上/新闻的数据源插件                    | OpenClaw 插件 (TS)   | Dune Analytics 连接器、FRED 宏观数据、社交情绪              |
| **人格 Personas**       | 不同角色的 Agent 人格模板                     | `SOUL.md`            | 保守长期投资者、激进加密短线手                              |
| **工作空间 Workspaces** | 完整工作空间配置（规则 + 人格 + 工具 + 定时） | 目录模板             | `workspace-crypto-trader/`、`workspace-retirement-planner/` |
| **知识包 Knowledge**    | 预构建的领域知识                              | Markdown / JSONL     | 美国税法要点、各国加密监管概览                              |
| **合规规则 Compliance** | 特定司法管辖区的监管规则                      | 结构化规则           | FATCA 申报门槛、MiFID II 适当性检查                         |

### 工作方式

**发布** — 贡献者打包作品并提交到 Commons：
...
```bash
# 搜索技能
finclaw commons search "投资组合再平衡"

# 安装工作空间模板
finclaw commons install workspace crypto-trader
```

**进化** — 社区评分、fork、改进。策略附带回测结果。技能展示使用统计。一切可版本化、可审计。

### 为什么叫「Commons」？

我们刻意选择 **Commons（共同体/公共资源）** 这个词 —— 不是「商店」或「市场」。金融智能应该是共享的公共资源，而非围墙花园。就像金融 AI 领域的维基百科，贡献的人越多，每个人的 Agent 就越聪明。
...
- [ ] 金融技能插件框架和金融领域 SKILL.md 规范
- [ ] CCXT 交易桥接（Hyperliquid、Binance、OKX、Bybit）
- [ ] 基础投资组合追踪与报告
- [ ] 用户金融画像记忆系统
- [ ] 向 Commons 发布首批 10 个金融技能和 3 个工作空间模板
...
- [ ] FinClaw Commons 网页门户（浏览、评分、fork）
- [ ] 第三方数据连接器市场
- [ ] DeFi 跨链操作
- [ ] 监管合规自动驾驶
- [ ] 各辖区合规规则集（社区维护）
...
3. **开放共同体** — 金融智能是公共资源。核心引擎、技能、策略和知识对所有人开放。
4. **安全护栏** — 交易操作始终需要用户明确授权，绝不静默交易。
5. **持续进化** — 技能和能力通过社区贡献和 Agent 自我迭代不断成长。
...
OpenFinClaw 是一个开放社区项目。**FinClaw Commons** 由像你一样的贡献者共同建设。

### 七种贡献方式

| 贡献什么         | 适合谁                    | 怎么做                                                |
| ---------------- | ------------------------- | ----------------------------------------------------- |
| **金融技能**     | 分析师、顾问、领域专家    | 为你的专业领域编写 `SKILL.md`（税务规划、风险分析等） |
| **交易策略**     | 量化开发者、交易员        | 打包一个带有历史表现数据的可回测策略                  |
| **数据连接器**   | 开发者、数据工程师        | 为金融数据源构建 OpenClaw 插件                        |
| **Agent 人格**   | 任何有金融领域洞察的人    | 编写一个 `SOUL.md` 来体现特定的投资哲学               |
| **工作空间模板** | 高级用户、金融从业者      | 分享针对特定用例的完整工作空间配置                    |
| **知识包**       | 研究者、法律/税务专业人士 | 策展领域知识（税法、监管规则、市场历史）              |
| **合规规则集**   | 法律/合规专业人士         | 文档化特定辖区的金融监管规则                          |

你也可以贡献**代码、文档、翻译、Bug 报告和功能建议** —— 一切都欢迎。
